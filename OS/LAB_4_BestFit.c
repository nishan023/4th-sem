...
void bestFit(int blocks[], int originalBlocks[], int m, int processes[], int n)
{
    int allocation[n];
    for (int i = 0; i < n; i++)
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < m; j++)
            if (blocks[j] >= processes[i] && blocks[j] - processes[i] < minDiff)
                minDiff = blocks[j] - processes[i], bestIdx = j;
        // If a suitable block is found, allocate it to the process
        if (bestIdx != -1)
            allocation[i] = bestIdx, blocks[bestIdx] -= processes[i];
    }
...
int main()
{
    ...
    // Execute the Best Fit memory allocation algorithm
    bestFit(blocks, originalBlocks, m, processes, n);
    return 0;
}
